...
### Bug Fixes 🐛
- Keep normal rotate-proxy retry decisions from immediately cooling down every proxy candidate; callers now opt into critical cooldown through `RetryDecision.ProxyFailureSeverity`.
...
### Testing 🧪
- Add crawler response coverage for normal rotate-proxy retry failures and explicit critical cooldown.
